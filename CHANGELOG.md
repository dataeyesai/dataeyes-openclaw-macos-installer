...
- 过滤安装时自动发现的模型列表，排除图片、视频、音频、预览、Embedding、Moderation 等非聊天模型
- 对模型家族做更严格筛选，减少把不适合 OpenClaw 聊天切换的模型注入配置
- 默认模型改为优先 `dataeyes/gpt-5.4`，不再优先沿用旧的 `kimi-k2`

## 1.0.6 - 2026-03-25

- 安装界面新增旧安装检测，能明确提示当前是首次安装还是覆盖升级
- 新增“彻底重装（清理旧版本后再安装）”开关
- 彻底重装会额外清理用户目录中的旧 OpenClaw 安装残留和安装器支持脚本
- 优化按钮初始状态，避免首次打开时展示与当前安装状态不一致
